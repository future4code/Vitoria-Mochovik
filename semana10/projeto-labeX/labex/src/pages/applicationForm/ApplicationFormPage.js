...
const ApplicationFormPage = () => {
    ...
    const getListTrips = () => {
        ...
        axios.get(url)
        .then((res) => {
            console.log("consegui", res.data.trips)
    ...
    const applyToTrip = (event) => {
        ...
        const url = `https://us-central1-labenu-apis.cloudfunctions.net/labeX/vitoria-mochovik-molina/trips/${form.id}/apply`

        axios.post(url, form)
        .then((res) => {
            alert("Inscrição concluída com sucesso!")
            console.log("aqui 003", res.data)
            cleanFields()
        })
        .catch((err) => {
            console.log("nãooo foii")
        })
    ...
    const nameTrips = trips.map((information) => {
        return(
            <option value={information.id} key={information.id}> 
                {information.name} 
            </option> 
        )
    })
